...
here::here(dir.create("raw_data"))

here::here(dir.create("figures"))

here::here(dir.create(paste("figures",
                            site, 
                            sep = "/")))
...
here::here(dir.create(paste("raw_data", 
                            i, 
                            sep = "/")))
...
write_csv(mydat3, here::here("raw_data",
                             "historical_daily",
                             site, 
                             paste("lat",lat,"lon",lon, sep = "_"),
...
for(climvar in c("soil_water_monthly", "runoff_monthly", "rain_monthly", "accumswe_monthly", "PET_monthly", "Deficit_monthly", "AET_monthly")){ 
  ...
  for(yr in c(start:end)){# for each climate variable loop across the year.  allows users to select range if not interested in all data 
    ...
    data_url<-paste("http://www.yellowstone.solutions/thredds/ncss/daily_or_monthly/v2_historical/gridmet_v_1_5_historical/V_1_5_",yr,"_gridmet_historical_",climvar,".nc4?var=",vshort,"&latitude=",lat,"&longitude=",lon,"&time_start=1980-01-16T05%3A14%3A31.916Z&time_end=1980-12-16T00%3A34%3A14.059Z&accept=csv_file",sep ="") 
    } #end if statement for gridmet data url
...
if(past_data == "gridmet"){
  write_csv(mydat3, here::here("raw_data",
                             ...
                             site, 
                             paste("lat",lat,"lon",lon, sep = "_"),
                             paste(site,"monthly", past_data, ".csv", sep = "_")))
}
