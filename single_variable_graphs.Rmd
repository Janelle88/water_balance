...
# 
 dry_year = 2013 # a particularly dry year, maybe with lots of fire activity? something for people to reference
# 
...
future_day_1 <- read_csv(here::here(paste(site,model_wc,model_wc_rcp, model_bc,model_bc_rcp, climvar, ".csv", sep = "_")),
                    na = c("-3276.7"))
...
past_day_1 <- read_csv(here::here(paste("historical_", climvar, ".csv", sep = "")),
                           na = c("-3276.7")) 
...
annual_past <- past_day %>% 
  group_by(year) %>% 
  filter(!year == 2020) %>% 
  summarize(annual_deficit = sum(deficit_daily, na.rm = TRUE) 
...
wc_annual_future <- future_day %>%
  select(deficit_daily_wc, date:doy) %>% # soil_water_daily_wc:aet_daily_wc, selecting for variables for 8.5 only
  filter(!year == 2100) %>% 
  group_by(year) %>% 
    summarize(annual_deficit_wc = sum(deficit_daily_wc, na.rm = TRUE))
...
bc_annual_future <- future_day %>% 
  select(deficit_daily_bc, date:doy) %>% #soil_water_daily_bc:aet_daily_bc,selecting for 4.5 only
  filter(!year == 2100) %>% 
  group_by(year) %>% 
    summarize(annual_deficit_bc = sum(deficit_daily_bc, na.rm = TRUE))
...
bc_annual_values <- annual_past %>%
  ...
  pivot_longer(`annual_deficit`:`annual_deficit_bc`,#`annual_soil_water`:`annual_aet_bc`, # The columns I'm gathering together
...
wc_annual_values <- annual_past %>% 
  ...
  pivot_longer(`annual_deficit`:`annual_deficit_wc`,#`annual_soil_water`:`annual_aet_wc`, # The columns I'm gathering together
               names_to = "variable", # new column name for existing names
               values_to = "annual_value") %>% 
  na.omit(annual_values) %>% # new column name to store values
  mutate(decade = case_when(
...
doy_avg_2020_2059_bc <- future_day %>% 
  ...
  select(deficit_daily_bc:doy) %>% 
  pivot_longer(deficit_daily_bc,
...
#soil_water_daily_bc:doy) %>%
  pivot_longer(`deficit_daily_bc`,#`soil_water_daily_bc`:`aet_daily_bc`, # The columns I'm gathering together
               names_to = "variable", # new column name for existing names
               values_to = "value") %>%  # new column name to store values) 
  mutate(variable = case_when(.$variable == "aet_daily_bc" ~ "aet_daily",
    .$variable == "rain_daily_bc" ~ "rain_daily",
...
doy_avg_2020_2059_wc <- future_day %>% 
  filter(year %in% c(2020:2059)) %>%
  select(deficit_daily_wc, date:doy) %>% #soil_water_daily_wc:doy) %>% 
  pivot_longer(`deficit_daily_wc`,#`soil_water_daily_wc`:`aet_daily_wc`, # The columns I'm gathering together
               names_to = "variable", # new column name for existing names
               values_to = "value") %>% # new column name to store values)
...
#   .y_plot_bc <- .y_plot_bc + geom_vline(aes(xintercept = xintercept), data = vline_data_bc)
  ...
  ggsave(here::here(paste(site, climvar, "40_yr",model_bc_rcp, model_bc, ".png", sep = "_")), width = 6, height = 4) #file name
  ...
  ggsave(here::here(paste(site, climvar, "40_yr",model_wc_rcp, model_wc, ".png", sep = "_")), width = 6, height = 4) #file name
...
plot_annual = function(.y) {
 ...
 ylim(NA, max_y) +
  ...
  ggsave(here::here(paste(site, climvar, model_bc_rcp, model_bc, "annual.png", sep = "_")), width = 6, height = 4) #file name
  ...
  ggsave(here::here(paste(site, climvar, model_wc_rcp, model_wc, "annual_graph_wc.png", sep = "_")), width = 6, height = 4) #filename
