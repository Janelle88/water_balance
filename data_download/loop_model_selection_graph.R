...
lon <- pull(lons, Long)
  ...
  coordinates(Site_coordinates) <- ~lon+lat # for some reason in the loop, these revert to how they are stored in the csv, need to write them that way
  ...
  # download data
  # make data download in chunks of historical data and future data
  # helps for pcs that cannot store all that memory
  # memory problems may cause the script to stop running
download_data <- function(startyear, endyear){#start download_data function
  ...
  }# end download_data function

  # Date, GCM,PrecipCustom, TmaxCustom, TminCustom, RHmaxCustom, RHminCustom,TavgCustom

# download baseline data
download_data(Hist_StartYear, Hist_EndYear)
# manipulate into data frame
Baseline_all <- as.data.frame(subset(df, select=c("Date", "GCM", "PrecipCustom", "TmaxCustom", "TminCustom","TavgCustom")))
#select=c("Date", "GCM", "PrecipCustom", "TmaxCustom", "TminCustom", "TavgCustom")

# "RHmaxCustom", "RHminCustom", were before tavgCustom

# download future data
download_data(Future_StartYear, Future_EndYear)
# manipulate into data frame
Future_all<-as.data.frame(subset(df, select=c("Date", "GCM", "PrecipCustom", "TmaxCustom", "TminCustom", "TavgCustom")))
# , select=c("Date","GCM", "PrecipCustom", "TmaxCustom", "TminCustom", "TavgCustom"))
