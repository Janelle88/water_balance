...
plot_decades_line("soil_water_daily")
plot_decades_line("runoff_daily")
plot_decades_line("rain_daily")
plot_decades_line("accumswe_daily")
plot_decades_line("pet_daily")
plot_decades_line("deficit_daily")
plot_decades_line("aet_daily")
...
colors <- c("Historical" = "gray60", "Best Case" = "turquoise4", "Worst Case" = "darkgoldenrod3")
...
ggplot() +
  geom_line(data = annual_past, aes(x = year, y = annual_avg, color ="Historical"),
            size = .6) + 
  geom_line(data = annual_future, aes(x = year, y = annual_avg_wc, color = "Worst Case"),
            size = .6) +
  geom_line(data = annual_future, aes(x = year, y = annual_avg_bc, color = "Best Case"),
            size = .6) +
  facet_wrap(~variable, scales = "free") +
  scale_color_manual(values = colors,
                     labels = "Historical", paste(model_bc, model_bc_rcp_name), paste(model_wc, model_wc_rcp_name)) +
...
plot_annual = function(.y, .x, .z, .l) { 
  ...
  annual_graph_bc <- ggplot() +
    geom_line(data = past_annual, # historical line
              aes(x = year, 
                  ...
                  color = "Historical"),
              size = .75) +
    geom_line(data = future_annual, # Best case line
              aes(x = year, 
                  y = annual_avg_bc, 
                  color = "RCP 4.5"),
              size = .75) +
  ...
  labs(color = "Annual Averages", #legend titel name
       title = case_when(.y ==  "soil_water_daily" ~ paste("Soil Water",
                                                            model_wc,
                                                          model_wc_rcp_name), #title based on variable
                           .y == "runoff_daily" ~ paste("Runoff",
                                                        model_wc,
                                                        model_wc_rcp_name),
                           .y == "rain_daily" ~  paste("Rain",
